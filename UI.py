...
def newRecord():
    while True:
        input_overall = input(f'''How would you rete your Overall feeling today?
        horrible -> {moodTracker.OVERALL_RATES} -> Excellent
        >''')
        if input_overall in moodTracker.OVERALL_RATES:
            break
    while True:
        input_anxiety = input(f'''How were your Anxiety levels today?
        {moodTracker.ANXIETY_RATES}
        >''')
        if input_anxiety in moodTracker.ANXIETY_RATES:
            break
    while True:
        input_mood = input(f'''How was your Mood today?
    ...
    input_exercise = set()
    while True:
        entry = input(f'''Did you have any exercise today? name it all. type 'Done' when finished.
    ...
    while True:
        input_reading = input(f'''How Much did you Read today?
        >''')
        try:
            ...
            continue
    while True:
        input_health = input(f'''How was your Health today?
        {moodTracker.HEALTH_RATES}
        >''')
        if input_health in moodTracker.HEALTH_RATES:
            break
    while True:
        input_phoneUse = input(f'''How was your phone/social media Use today?
